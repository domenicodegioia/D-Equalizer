...
struct Fifo
{
    ...
    {
        //static_assert( std::is_same<T, juce::AudioBuffer<float>>, "prepare(numChannels, numSamples) should only be used when the Fifo is holding juce::AudioBuffer<float>");
    ...
    void prepare(size_t numElements)
        {
        //static_assert( std::is_same<T, std::vector<float>>, "prepare(numElements) should only be used when the Fifo is holding std::vector<float>");
            for( auto& buffer : buffers )
            {
                buffer.clear();
...
//A CutFilter is the series of four single Filter.
//We have four different slopes for our CutFilter.
